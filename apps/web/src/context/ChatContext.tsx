...
const GATEWAY_URL =
  !rawGatewayUrl.startsWith('http://') && !rawGatewayUrl.startsWith('https://')
...
const DEMO_CONVERSATIONS: Conversation[] = [
  {
    ...
    lastMessage: {
      content: "Hey! How's it going today?",
      timestamp: '09:15 AM',
      senderId: 'u1',
    },
    unreadCount: 4,
  },
  {
    ...
    lastMessage: {
      content: 'Can you help me figure this out?',
      timestamp: '09:22 AM',
      senderId: 'u2',
    },
    unreadCount: 0,
  },
  {
    ...
    lastMessage: {
      content: "OMG, you won't believe what just happened!",
      timestamp: '09:30 AM',
      senderId: 'u3',
    },
    unreadCount: 0,
  },
  {
    ...
    lastMessage: {
      content: 'Do you have a minute to talk?',
      timestamp: '09:45 AM',
      senderId: 'u4',
    },
    unreadCount: 1,
  ...
  u1: [
    {
      ...
      senderId: 'u1',
      recipientId: 'me',
      content: 'Hi there! Did you see the new designs?',
      createdAt: '09:10 AM',
      status: 'read',
    },
    {
      ...
      senderId: 'me',
      recipientId: 'u1',
      content: 'Yes! They look super clean. Love the orange accents.',
      createdAt: '09:12 AM',
      status: 'read',
    },
    {
...
export function ChatProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    roomIdMapRef.current = roomIdMap;
  }, [roomIdMap]);
  ...
  const refreshRoomsRef = useRef<(silent?: boolean) => Promise<void>>(
    async () => {},
  );
  ...
  const refreshRooms = async (silent = false) => {
    ...
    try {
      ...
      const convs: Conversation[] = rooms.map((room) => {
        const isSelfRoom =
          ...
          (room.members?.length > 0 &&
            room.members.every((m: any) => m.userId === user.id));
        ...
        return {
          user: {
            ...
            name: isSelfRoom
              ? 'Saved Messages'
              : otherMember?.name || room.name || 'Unknown',
            email: isSelfRoom ? user.email || '' : otherMember?.email || '',
            profile_picture: isSelfRoom
      ...
      presenceSnapshotRef.current.forEach((id) => {
        presenceMap[String(id)] = true;
      });

      if (otherUserIds.length > 0) {
        try {
          const presenceList = await apiRequest<
          ...
          presenceList.forEach((p) => {
            presenceMap[String(p.userId)] = p.isOnline;
          });
        } catch {
      ...
      const enrichedConvs = convs.map((c) => {
        const enriched = {
          ...
          user: {
            ...c.user,
            isOnline: presenceMap[String(c.user.id)] ?? false,
          },
        };
    ...
    try {
      ...
      const msgs: Message[] = (data.messages || []).map((m: any) => {
        ...
        return {
          ...
          senderId,
          recipientId:
            m.recipientId || (senderId === user.id ? conv.user.id : user.id),
          content: m.content,
          fileUrl: m.isDeleted ? undefined : m.fileUrl,
          messageType: m.messageType || 'text',
          createdAt: rawDate.toLocaleTimeString([], {
            hour: '2-digit',
            minute: '2-digit',
          }),
  ...
  const deleteMessage = async (messageId: string) => {
    ...
    const roomId =
      ...
      roomIdMapRef.current[activeConversation.user.id];
    if (!roomId) return;
  ...
  const editMessage = async (messageId: string, newContent: string) => {
    ...
    setMessagesMap((prev) => {
      ...
      return {
        ...
        [activeConversation.user.id]: current.map((m) =>
          m.id === messageId
            ? { ...m, isEdited: true, content: newContent }
            : m,
        ),
      };
    });
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener('chat:refresh-messages', handleRefresh);
  ...
  useEffect(() => {
    if (user) {
      ...
      apiRequest<string[]>('/user/blocked-users')
        .then((list) => {
          if (Array.isArray(list)) setBlockedUserIds(list);
        })
        .catch(() => {});
    } else {
  ...
  useEffect(() => {
    ...
    import('socket.io-client').then(({ io }) => {
      ...
      socket.on('presence_snapshot', (payload: { onlineUserIds: string[] }) => {
        console.log(
          '[Socket] presence_snapshot received:',
        ...
        setActiveConversation((prev) => {
          ...
          const updated = {
            ...
            user: {
              ...prev.user,
              isOnline: onlineSet.has(String(prev.user.id)),
            },
          };
      ...
      const handleNewMessage = (payload: {
        ...
        if (!conv) {
          console.warn(
            '[Socket] new_message: roomId not found in conversations:',
            payload.roomId,
          );
          return;
        }
        console.log(
          '[Socket] new_message: matched conversation with user:',
          conv.user.id,
          conv.user.name,
        );
        ...
        setMessagesMap((prev) => {
          ...
          console.log(
            '[Socket] new_message: updating messagesMap for userId:',
            conv.user.id,
            'duplicate?',
            isDuplicate,
            'existing count:',
          ...
          if (isDuplicate) {
            console.log(
              '[Socket] new_message: Duplicate detected, skipping update',
            );
            return prev;
          }
          const updated = { ...prev, [conv.user.id]: [...existing, msg] };
          console.log(
            '[Socket] new_message: Added message, new count:',
            updated[conv.user.id].length,
          );
          return updated;
        });
      ...
      socket.on(
        ...
        (payload: { userId: string; roomId?: string }) => {
          console.log(
            '[Socket] messages_read received from userId:',
            payload.userId,
          );
      ...
      socket.on(
        'member_removed',
        (payload: { roomId: string; userId: string; removedBy: string }) => {
          ...
          if (payload.userId === user.id) {
            ...
            setConversations((prev) =>
              prev.filter((c) => c.roomId !== payload.roomId),
            );
          } else {
            ...
            prev.map((c) => {
              ...
              return {
                ...
                user: {
                  ...
                  profile_picture:
                    payload.updates.group_picture ?? c.user.profile_picture,
                },
              };
            }),
          );
          ...
          setActiveConversation((prev) => {
            ...
            return {
              ...
              user: {
                ...
                profile_picture:
                  payload.updates.group_picture ?? prev.user.profile_picture,
              },
            };
          });
        },
      ...
      socket.on(
        'group_created',
        (payload: { roomId: string; room: any; createdBy: string }) => {
  ...
  const selectConversation = (conversation: Conversation | null) => {
    ...
    const fresh =
      conversationsRef.current.find(
        (c) => c.user.id === conversation.user.id,
      ) || conversation;
    setActiveConversation(fresh);
    setConversations((prev) =>
      prev.map((c) =>
        c.user.id === fresh.user.id ? { ...c, unreadCount: 0 } : c,
      ),
    );
    if (user) {
      loadMessages(fresh);
  ...
  const sendTyping = (isTyping: boolean) => {
    ...
    const roomId =
      activeConversation.roomId || roomIdMapRef.current[recipientId];
  ...
  const sendMessage = async (
    ...
    setMessagesMap((prev) => ({
      ...
      [activeConversation.user.id]: [
        ...(prev[activeConversation.user.id] || []),
        optimisticMsg,
      ],
    }));

    const snippetText = fileUrl
      ? messageType === 'video'
        ? '📹 Video'
        : '📷 Photo'
      : content.trim();
    ...
    if (roomId) {
      try {
        ...
        setMessagesMap((prev) => ({
          ...
          [activeConversation.user.id]: (
            prev[activeConversation.user.id] || []
          ).map((m) => (m.id === tempId ? { ...m, status: 'delivered' } : m)),
        }));
      } catch (err) {
  ...
  const unblockUser = async (userId: string) => {
    try {
      ...
      setConversations((prev) =>
        prev.map((c) =>
          c.user.id === userId ? { ...c, isBlocked: false } : c,
        ),
      );
      setActiveConversation((prev) =>
  ...
  const archiveConversation = (userId: string) => {
    setArchivedConversationIds((prev) => {
      ...
      try {
        localStorage.setItem('archived_conv_ids', JSON.stringify(next));
      } catch {}
      return next;
    });
